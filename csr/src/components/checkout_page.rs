...
use crate::{
    canister::backend::{CarAvailability, CarStatus, CustomerDetials, RentalTransaction},
    components::{ActionTrackerPopup, Footer},
    state::{
        ...
        Some(car) => {
            ...
            let create_action = create_action(move |&()| {
                ...
                async move {
                    ...
                    let cans = cans.canisters().unwrap();
                    let backend = cans.backend().await;
                    let reserve = backend
                        .reserve_car(
                            car.id,
                            checkout.start_time.get_untracked().unwrap(),
                            checkout.end_time.get_untracked().unwrap(),
                            CustomerDetials {
                    ...
                    match reserve {
                        Ok(result1) => match result1 {
                            crate::canister::backend::Result1::Ok(rental_transaction) => {
                                Ok(rental_transaction)
                            }
            ...
            let create_diabled =
                create_memo(move |_| creating.get() || !user.user.get().check_ready());
                ...
                Err(e) => {
                    view! {
                        <div class="flex justify-center">
                            ...
                            <div class="flex justify-center">
                                <button
                                    on:click=move |_| close_popup.set_untracked(false)
