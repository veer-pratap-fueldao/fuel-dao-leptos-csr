pub mod server_impl; // Declare the module without the SSR conditional compilation
...
pub fn generate_anonymous_identity_if_required() -> Result<Option<JwkEcKey>, ServerFnError> {
    server_impl::generate_anonymous_identity_if_required_impl() // Call the implementation directly
}

/// This function is purely a side effect and only sets the refresh token cookie
pub fn set_anonymous_identity_cookie(anonymous_identity: JwkEcKey) -> Result<(), ServerFnError> {
    server_impl::set_anonymous_identity_cookie_impl(anonymous_identity) // Call the implementation directly
}

/// Extract the identity from the refresh token,
/// returns None if the refresh token doesn't exist
pub fn extract_identity() -> Result<Option<DelegatedIdentityWire>, ServerFnError> {
    server_impl::extract_identity_impl() // Call the implementation directly
}

pub fn logout_identity() -> Result<DelegatedIdentityWire, ServerFnError> {
    server_impl::logout_identity_impl() // Call the implementation directly
}
